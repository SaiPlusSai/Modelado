...
export default function LcgAditivo() {
  const [params, setParams] = useState({ a: 13, c: 7, m: 8, seed: 5, n: 9 });
  ...
  const onSubmit = (e) => {
    ...
    const seq = lcgSequence({
      a: params.a, c: params.c, m: params.m, seed: params.seed, n: params.n, normalized: true
    });
    setResult(seq);
  };
  ...
  return (
    <section className="card">
      <h3 style={{ marginBottom: 12 }}>LCG Aditivo (c ≠ 0)</h3>
      <form onSubmit={onSubmit} className="grid-form">
        <label> a
          <input type="number" name="a" value={params.a} onChange={onChange} />
        </label>
        <label> c
          <input type="number" name="c" value={params.c} onChange={onChange} />
        </label>
        <label> m
